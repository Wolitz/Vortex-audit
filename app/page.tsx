...
import { 
  Clock3, Target, BookOpen, ArrowLeft, UploadCloud, FileVideo, X, CheckCircle, Check,
  AlertTriangle, Sparkles, Zap, BrainCircuit, Mic, FileText, BotMessageSquare, ShieldCheck, Trash2, PlayCircle, Download, LogOut
} from "lucide-react";

import { MAX_FILE_SIZE_BYTES, MAX_FILE_SIZE_MB } from "@/lib/pricing";
...
export default function Home() {
  ...
  const [auditProgress, setAuditProgress] = useState(0); // <-- NEW STATE ADDED
...
=========================================
  ...
  const runAudit = async () => {
    ...
    if (file.size > MAX_FILE_SIZE_BYTES) {
      alert(`This video is ${(file.size / (1024 * 1024)).toFixed(0)}MB. The limit is ${MAX_FILE_SIZE_MB}MB.`);
      return;
    }

    setIsAuditing(true);
    setAuditProgress(5); // Start the loading bar

    let uploadInterval: NodeJS.Timeout | undefined;
    let aiInterval: NodeJS.Timeout | undefined;

    try {
      // 1. Simulate progress during Blob upload
      uploadInterval = setInterval(() => {
        setAuditProgress(prev => prev < 35 ? prev + 2 : prev);
      }, 300);

      console.log("Uploading to Vercel Blob...");
      const safeFileName = file.name.replace(/[^a-zA-Z0-9.-]/g, '_');
      // The upload route only issues tokens for this user's own prefix.
      const uploadPath = `uploads/${session.user.id}/${Date.now()}-${safeFileName}`;

      let blobPathname = uploadPath;

      if (BLOB_ACCESS === "private") {
        // Presigned PUT: no Authorization header, so vercel.com/api/blob CORS
        // does not block custom domains the way client-token uploads do.
        const tokenResponse = await fetch("/api/upload", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ pathname: uploadPath }),
        });
        const tokenData = await tokenResponse.json().catch(() => ({}));
        if (!tokenResponse.ok) {
          throw new Error(tokenData.error || "Could not start the upload.");
        }

        const putResponse = await fetch(tokenData.url as string, {
          method: "PUT",
        ...
        const newBlob = await uploadToBlob(uploadPath, file, {
          access: "public",
      ...
      // 2. Simulate progress during the AI Audit process
      aiInterval = setInterval(() => {
        setAuditProgress(prev => prev < 98 ? prev + 1 : prev);
      }, 500);

      console.log("Sending upload reference to the audit engine...");
      ...
      if (!response.ok) {
        if (response.status === 429) setShowPricing(true);
        throw new Error(data.error || "Audit failed");
      }

      setAuditProgress(100); // Lock it at 100% on success
      ...
      setAuditProgress(0); // Reset on error
    } finally {
      // Ensure all intervals are cleaned up
      if (uploadInterval) clearInterval(uploadInterval);
      if (aiInterval) clearInterval(aiInterval);
      
      // Delay resetting the UI so the user can see it hit 100%
      setTimeout(() => {
        setIsAuditing(false);
        setAuditProgress(0);
      }, 700);
  ...
  // ==========================================
  return (
    <main className="min-h-screen bg-[#060606] text-[#E0E0E0]">
      ...
      <div className="flex h-screen w-full relative overflow-hidden">
        ...
        <Link href="/" className="flex items-center space-x-3 group cursor-pointer w-fit">
            <img 
              src="/logo.svg" 
              alt="WOB Logo" 
              className="w-7 h-7 object-contain group-hover:animate-pulse transition-all duration-300" 
            />
            <h1 className="text-2xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400 group-hover:to-[#2DD4BF] transition-all duration-300">
              WOB
            </h1>
        </Link>
                          ...
                          <div className="flex items-end justify-between">
                            ...
                            {!auditResult && (
                              <button 
                                onClick={runAudit}
                                disabled={isAuditing}
                                className="flex items-center space-x-2.5 bg-[#E0E0E0] text-black px-6 py-3.5 rounded-xl font-medium hover:bg-white transition-colors shadow-lg disabled:cursor-not-allowed relative overflow-hidden group h-fit min-w-[240px] justify-center"
                              >
                                {/* 🚀 THE FIX: Dynamic filling progress bar background */}
                                {isAuditing && (
                                  <div 
                                    className="absolute left-0 top-0 bottom-0 bg-[#A78BFA]/40 transition-all duration-300 ease-out z-0"
                                    style={{ width: `${auditProgress}%` }}
                                  ></div>
                                )}
                                
                                <BrainCircuit size={18} className={`relative z-10 ${isAuditing ? "text-[#5e4999]" : ""}`} />
                                <span className="relative z-10">
                                  {isAuditing 
                                    ? `Analyzing Manifest... ${Math.round(auditProgress)}%` 
                                    : "Run Audit Manifest"
                                  }
                                </span>
                              </button>
